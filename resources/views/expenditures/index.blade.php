        ...
        @foreach($budget as $b)
                ...
                @foreach($subcategory_totals as $st)
                        @if($b->subcategory === $st->subcategory)
                                <td align="right">$ {{number_format($st->sum1,2,".",",")}}
                        <td align="right">$ {{number_format($b->amount,2, ".", ",")}}<td align="right"> {{number_format($st->sum1*100/$b->amount, 2, ".",",")}}%
                        @endif
                @endforeach
...
<tr><td colspan=2><strong>Total<td align="right">$ {{number_format($total_spent,2)}}<td align="right">$ {{number_format($total,2)}}<td align="right">{{number_format(100*$total_spent/$total, 2)}}%</strong>
</table>
...
<table class="table">
        ...
        <tr><td>{{$expenditure->date}}<td>{{$expenditure->subcategory}}<td align="right">$ {{number_format($expenditure->amount,2)}}<td>{{$expenditure->description}}
        @endforeach
</table>
@stop
